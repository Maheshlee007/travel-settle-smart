...
export const LodgingTab = ({ importedExpenses = [] }: LodgingTabProps) => {
  const [rows, setRows] = useState<LodgingRow[]>([
    { id: "1", date: "", hotelName: "", city: "", amount: 0, bill: null, remarks: "" },
  ]);
  ...
  useEffect(() => {
    if (importedExpenses.length > 0) {
      const importedRows = importedExpenses.map(exp => ({
        id: exp.id,
        date: exp.date,
        hotelName: "", // User will need to fill these
        city: "",
        amount: exp.amount,
        bill: exp.image,
        remarks: exp.remarks
  ...
  const addRow = () => {
    setRows([...rows, { id: Date.now().toString(), date: "", hotelName: "", city: "", amount: 0, bill: null, remarks: "" }]);
  };
  ...
  const updateRow = (id: string, field: keyof LodgingRow, value: any) => {
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="min-w-[240px]">Hotel Name</TableHead>
            ...
            {rows.map((row) => (
              <TableRow key={row.id}>
                ...
                <TableCell>
                  <Input
                    value={row.hotelName}
                    onChange={(e) => updateRow(row.id, "hotelName", e.target.value)}
